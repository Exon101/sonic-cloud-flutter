...
void main() {
  ...
  group('PlaybackService', () {
    ...
    test('load() sets currentUrl and calls setUrl on the player', () async {
      ...
      verify(() => mockPlayer.setUrl('asset:///assets/audio/sample_track.wav'))
          .called(1);
      expect(service.currentUrl, 'asset:///assets/audio/sample_track.wav');
    });

    test('load() is idempotent for the same URL — does not call setUrl twice',
        () async {
    ...
    test('seekToProgress computes the right Duration from a fraction',
        () async {
